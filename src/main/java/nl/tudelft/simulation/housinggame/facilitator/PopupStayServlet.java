...
@WebServlet("/popup-stay")
public class PopupStayServlet extends HttpServlet
{
    /** */
            ...
            throws ServletException, IOException
    {
        ...
        if (request.getParameter("playerCode") != null && request.getParameter("transactionId") != null)
        {
            try
            {
                ...
                s.append("  <label for=\"comment-stay\">Comment:</label>\n");
                s.append("  <input type=\"text\" id=\"comment-stay\" name=\"comment-stay\" class=\"form-control\" "
                        + "style=\"z-index: 1000 !important;\" />\n");
                s.append("</div>\n");

                if (approve.equals("APPROVE"))
                {
                    String title = "Approve stay of house " + house.getCode() + " by " + playerCode;
                    String content = s.toString();
                    String method = "approveStay('" + playerCode + "', " + transactionId + ");";
                    ModalWindowUtils.makeModalWindowMethod(data, title, content, "APPROVE", method);
                }
                else
                {
                    String title = "Reject stay of house " + house.getCode() + " by " + playerCode;
                    String content = s.toString();
                    String method = "rejectStay('" + playerCode + "', " + transactionId + ");";
            ...
            {
                System.err.println("Error in popup-stay: " + e.getMessage());
            }
        }
        else
        {
            System.err.println("approve-stay called, but no playerCode or transactionId");
        }
